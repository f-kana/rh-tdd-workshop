...
function parsePrintingPage(inputText) {
    let result = [];
    inputText.split(',').forEach(element => {
        result = result.concat(parseCommaSplitPrintingPage(element));
    });
    return result;
}

/// カンマは処理対象外。単独数字とハイフンはうまく処理してた。
function parseCommaSplitPrintingPage(inputText) {
    if (!inputText.includes('-')) {
        return [Number(inputText)];
    }
    const [start, end] = inputText.split("-").map(Number);
